/* Xay dung ham duyet cay theo cach khu de quy nhung khong dung stack */
...
// s = "NLR" hoac "LNR" hoac "LRN" hoac "NRL" hoac "RNL" hoac "RLN"
void duyetCayKhongDeQuyKhongStack(NODE *root, char *s) {
  int length = strlen(s);
  if (length != 3)
    cout << "\nChuoi phep duyet khong hop le. Xin kiem tra lai";
  else { // hoan toan hop le
    int dem_N = 0;
    int dem_L = 0;
    int dem_R = 0;
    ...
    if (dem_N != 1 || dem_L != 1 || dem_R != 1)
     cout << "\nChuoi phep duyet khong hop le. Xin kiem tra lai";
      ...
      while (true) {
        ...
        else { // khi di vao day tuc la 1 node da di het thang thu tu duyet roi, luc nay khong di toi duoc nua ma phai lui ve cha cua no de xet theo huong khac
          root->thuTuDuyet = 0; // truoc khi tro ve cha -> reset lai thu tu duyet cua node do ve 0 de co the sau ham nay con co the duyet tiep ham khac nua
          root = root->cha;
          if (root == NULL)
            break; // DIEU KIEN DNG -> TU GOC TRO VE CHA SE LA NULL -> DUNG LAI
        }
      }
    }
  }
...
// Do thoi gian
void duyetCayKhongDeQuyKhongStack_DoThoiGian(NODE *root, char *s) {
  // int length = strlen(s);
  // if (length != 3)
  //   cout << "\nChuoi phep duyet khong hop le. Xin kiem tra lai";
  // else { // hoan toan hop le
...
/* -------------- DO THOI GIAN ----------------- */
  ...
  while (true) {
    ...
    NODE *p_left = taoNode(2 * p->data);
    p->left = p_left;
    p_left->cha = p;

    NODE *p_right = taoNode(2 * p->data);
    p->right = p_right;
    p_right->cha = p;
  ...
  // duyetCayKhongDeQuyKhongStack_DoThoiGian(root, (char *) "RLN");
  // duyetCayKhongDeQuyKhongStack_DoThoiGian(root, (char *) "LNR");
  // duyetCayKhongDeQuyKhongStack_DoThoiGian(root, (char *) "LRN");
  ...
  clock_t start2 = clock();
  for (int i = 0; i < 100000; i++)
    duyetCayKhongDeQuyKhongStack_DoThoiGian(root, (char *) "NLR");
  clock_t end2 = clock();
  cout << "\nduyetCayKhongDeQuyKhongStack_DoThoiGian chay mat " << (double)(end2 - start2) / CLOCKS_PER_SEC << " giay";

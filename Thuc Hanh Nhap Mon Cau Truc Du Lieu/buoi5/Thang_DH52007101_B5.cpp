...
void splitList_SinhVien(Nodeptr &list, Nodeptr &list1, Nodeptr &list2)
{
    ...
    {
        if (list->data.diem < 5)
            themCuoi(list1, list->data);
        else
            themCuoi(list2, list->data);
        list = list->link;
    }
}
...
void nhapThemDiem(Nodeptr &list)
{
    ...
    while (p != NULL)
    {
        cout << "Nhap them diem cho ma so sinh vien " << p->data.masv << endl;
        cout << "Nhap diem toan: ";
        cin >> p->data.toan;
        cout << "Nhap diem ly: ";
        cin >> p->data.ly;
        ...
        cout << "Diem trung binh cua sinh vien la: " << p->data.dtb << endl;
        p = p->link;
    }
}
void sapXepDanhSach(Nodeptr &list)

...
void QuickSort_Giam_Gia(List& l) {
  List l1, l2;
  Node* tag, * p;
  ...
  while (l.pHead != NULL) {
    p = l.pHead;
    l.pHead = p->pNext;l.pHead->pPrev;
    p->pNext = NULL; p->pPrev = NULL;
    if (p->data.gia <= tag->data.gia) AddHead(l1, p);
    else AddHead(l2, p);
  }
  QuickSort_Giam_Gia(l1);
  QuickSort_Giam_Gia(l2);
  if (l1.pHead != NULL) {
    l.pHead = l1.pHead;
    l1.pTail->pNext = tag;
    tag->pPrev = l1.pTail;
...
void QuickSort_Tang_Gia(List& l) {
  List l1, l2;
  Node* tag, * p;
  if (l.pHead == l.pTail) return;
  InitList(l1);InitList(l2);
  ...
  QuickSort_Tang_Gia(l1);
  QuickSort_Tang_Gia(l2);
  if (l1.pHead != NULL) {
    l.pHead = l1.pHead;
    l1.pTail->pNext = tag;
    tag->pPrev = l1.pTail;
...
void Menu(int& chon) {
  ...
  printf("\n |========|=================================================|");
  printf("\n |  \\18\\  |    \tQuickSort_giam_Gia!                         |");
  printf("\n |========|=================================================|");
  printf("\n |  \\19\\  |    \tQuickSort_tang_Gia!                         |");
  printf("\n |========|=================================================|");
  printf("\n |  \\0\\   |    \t\t\tThoat!                      |");
  printf("\n |========|=================================================|");
...
int main() {
  ...
  do {
    ...
    case 18:
      printf("\n\nSap xep tang gia \n");
      QuickSort_Tang_Gia(l);
      XuatDS(l);
      break;
    case 19:
      printf("\n\nSap xep giam gia \n");
      QuickSort_Giam_Gia(l);
      XuatDS(l);
      break;
    default: chon = 0;
    }
